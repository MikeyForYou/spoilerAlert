...
var mainFunction = function(obj){
	if(typeof(obj) == "object"){
		...
		$(".messageCozy-2JPAPA .markup-2BOw-j").each(function() {
...
spoilerAlert.prototype.hideSpoiler = function(isinit,specific,object){
	if(isinit){
		...
		injectStyle("\
			.messageCozy-2JPAPA .markup-2BOw-j.spoilerAlert{filter: blur(5px)}\
			.messageCozy-2JPAPA .markup-2BOw-j.spoilerAlert:hover{transition: 0.2s ease-in;filter: blur(0px)}\
		")
	}
	if(!specific){
...
spoilerAlert.prototype.observer = function(){
	this.hideSpoiler(false,true,$(".messageCozy-2JPAPA .markup-2BOw-j")[$(".messageCozy-2JPAPA .markup-2BOw-j").length-1]);
}
...
spoilerAlert.prototype.getDescription = function(){
		return "Blurs out text prefixed with \"!!\" for a spoiler free experience! Updated by Satoru";
};
